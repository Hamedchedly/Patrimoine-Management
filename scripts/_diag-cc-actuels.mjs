...
const { data: mx } = await db.from("travaux_commandes").select("annee_exercice").order("annee_exercice", { ascending: false }).limit(1);
const anneeMax = mx?.[0]?.annee_exercice;
console.log("ANNEE MAX:", anneeMax);

const { data: cmd } = await db.from("travaux_commandes").select("tranche_code, charge_clientele").eq("annee_exercice", anneeMax).not("charge_clientele","is",null);
const freq = new Map();
for (const c of cmd ?? []) { const cc = String(c.charge_clientele ?? "").trim(); if (!cc) continue; const k = `${c.tranche_code}::${cc}`; freq.set(k, (freq.get(k) ?? 0) + 1); }
...
for (const [k, n] of freq.entries()) {
  ...
  const m = parSS.get(s); m.set(cc, (m.get(cc) ?? 0) + n);
}
...
const { data: ccTable } = await db.from("psp_charges_clientele").select("sous_secteur, charge_clientele, identifiant_personnel, actif").order("sous_secteur");
