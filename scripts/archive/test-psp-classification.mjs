...
for (const [lib, attendu] of casType) {
  const r = cls(lib);
  check(
    `type «${lib.slice(0, 40)}» → ${attendu}`,
    r.type_intervention === attendu,
    r.type_intervention,
  );
}
...
check(
  "REMPLACEMENT COMPLET ASCENSEUR → ascenseur",
  cls("REMPLACEMENT COMPLET ASCENSEUR 2025").domaine_technique === "ascenseur",
);
const treuil = cls("REMPLACEMENT DE TREUIL");
check(
  "REMPLACEMENT DE TREUIL → ascenseur",
  treuil.domaine_technique === "ascenseur",
  treuil.domaine_technique,
);
...
check(
  "REMPLACEMENT DE DAD → SSI",
  dad.domaines_detectes.includes("ssi"),
  dad.domaines_detectes.join(","),
);
check("REMPLACEMENT DE DAD → validation", dad.besoin_validation_humaine === true);
const bal = cls("REMPLACEMENT DES BAL");
check(
  "REMPLACEMENT DES BAL → SSI (pas menuiserie)",
  bal.domaines_detectes.includes("ssi") && !bal.domaines_detectes.includes("menuiserie"),
  bal.domaines_detectes.join(","),
);
...
check(
  "VITRINES/PORTE AUTO/RIDEAUX → jamais plomberie",
  !vitrines.domaines_detectes.includes("plomberie"),
  vitrines.domaines_detectes.join(","),
);
check(
  "VITRINES/PORTE AUTO/RIDEAUX → menuiserie ou serrurerie",
  vitrines.domaines_detectes.includes("menuiserie") ||
    vitrines.domaines_detectes.includes("serrurerie_acces"),
  vitrines.domaines_detectes.join(","),
);
check(
  "VITRINES/PORTE AUTO/RIDEAUX → multi_domaine + validation",
  vitrines.domaine_technique === "multi_domaine" && vitrines.besoin_validation_humaine === true,
  vitrines.domaine_technique,
);
const ballon = cls("REMPLACEMENT BALLON THERMODYNAMIQUE");
check(
  "BALLON THERMODYNAMIQUE → chauffage (pas multi)",
  ballon.domaine_technique === "chauffage" && !ballon.domaines_detectes.includes("plomberie"),
  ballon.domaine_technique + " / " + ballon.domaines_detectes.join(","),
);
const chaud = cls("REMPLACEMENTS DE 6 CHAUDIÈRES VMC GAZ");
check(
  "CHAUDIÈRES VMC GAZ → multi (chauffage+ventilation)",
  chaud.domaine_technique === "multi_domaine" &&
    chaud.domaines_detectes.includes("chauffage") &&
    chaud.domaines_detectes.includes("ventilation"),
  chaud.domaines_detectes.join(","),
);
...
check(
  "DIAG TOITURE → multi (couverture+étanchéité) + validation",
  diagToit.besoin_validation_humaine === true && diagToit.domaine_technique === "multi_domaine",
  diagToit.domaine_technique,
);
const adapt = cls("ADAPT PMR - TRAVAUX SDB");
check(
  "ADAPT PMR - TRAVAUX SDB → plomberie + validation",
  adapt.domaines_detectes.includes("plomberie") && adapt.besoin_validation_humaine === true,
  adapt.domaines_detectes.join(","),
);
const revsol = cls("REMPLACEMENT REV SOL ESPACES COMMUNS");
check(
  "REV SOL ESPACES COMMUNS → parties communes + validation",
  revsol.famille_psp === "parties_communes" && revsol.besoin_validation_humaine === true,
  revsol.famille_psp,
);
...
check(
  "ATTESTATION GAZ → contrôle",
  attGaz.type_intervention === "controle",
  attGaz.type_intervention,
);
check(
  "ATTESTATION GAZ → chauffage (pas plomberie/multi)",
  attGaz.domaine_technique === "chauffage" && !attGaz.domaines_detectes.includes("plomberie"),
  attGaz.domaine_technique + "/" + attGaz.domaines_detectes.join(","),
);
const attElec = cls("ATTESTATION ELECTRIQUE");
check(
  "ATTESTATION ELECTRIQUE → électricité (pas plomberie/multi)",
  attElec.domaine_technique === "electricite" && !attElec.domaines_detectes.includes("plomberie"),
  attElec.domaine_technique + "/" + attElec.domaines_detectes.join(","),
);
...
// Relais de Chelles
const relais = cls(
  "RÉHABILITATION DU RELAIS DE CHELLES - 169 LOGEMENTS - LOT 01 : TRAITEMENT DES FAÇADES",
);
check("RELAIS DE CHELLES LOT 01 → réhabilitation", relais.type_intervention === "rehabilitation");
check(
  "RELAIS DE CHELLES LOT 01 → commande_exceptionnelle",
  relais.nature_exceptionnelle === "commande_exceptionnelle",
);
check("RELAIS DE CHELLES LOT 01 → validation", relais.besoin_validation_humaine === true);
check(
  "RELAIS DE CHELLES LOT 01 → domaine façade",
  relais.domaine_technique === "facade",
  relais.domaine_technique,
);
...
check(
  "WPATRIMOINE ER.T → tranche",
  cls("REMPLACEMENT DE VELUX", { patrimoine: "ER.T1396" }).element_patrimonial === "tranche",
);
check(
  "WPATRIMOINE ER.B → batiment",
  cls("REMPLACEMENT DE VELUX", { patrimoine: "ER.B1400.002" }).element_patrimonial === "batiment",
);

// Confiance ≥ 0.90 pour cas direct non ambigu
check(
  "confiance 0.90 (direct non ambigu)",
  cls("REMPLACEMENT DE VELUX").confiance === 0.9,
  String(cls("REMPLACEMENT DE VELUX").confiance),
);
...
if (!imps || imps.length === 0) {
  ...
  const { data: rows, error } = await db
    ...
    .select(
      "numero_commande_interne, numero_commande, patrimoine, nature_analytique, corps_etat_libelle, montant_engage, donnees_brutes",
    )
  ...
  check(
    "407 COMN_NUM distincts (aucune fusion)",
    new Set(comns).size === 407 && comns.length === 407,
    `${new Set(comns).size}/${comns.length}`,
  );
  check(
    "aucun COMN_NUM null",
    comns.every((c) => c != null && String(c).trim() !== ""),
  );
  ...
  check(
    "aucune ligne perdue (407 classifiées)",
    classifications.length === 407,
  ...
  const nIndet = classifications.filter(
    (c) => c.domaine_technique === "indetermine" || c.type_intervention === "indetermine",
  ).length;
  const nAscenseur = classifications.filter((c) =>
    c.domaines_detectes.includes("ascenseur"),
  ).length;
  ...
  check(
    "total occurrences des groupes = nb validations",
    totalGroupes === nVal,
    `${totalGroupes}/${nVal}`,
  );
