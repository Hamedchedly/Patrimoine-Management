...
async function main() {
  ...
  const { data: lots } = await db
    .from("lots")
    .select("id, tranche_code, code_patrimoine")
    .eq("actif", true)
    .order("code_patrimoine")
    .limit(60);
  if ((tranches ?? []).length < 1 || (lots ?? []).length < 2) {
    console.error(
      "Données patrimoniales insuffisantes pour le test V7 (besoin de 1 tranche + 2 lots).",
    );
    process.exit(1);
  }
  ...
  if (lotsA.length < 2) {
    console.error(
      `La tranche ${trancheA} n'a pas 2 lots — choisir une autre tranche manuellement.`,
    );
    process.exit(1);
  }
  const lotA1 = lotsA[0];
  ...
  const { data: perCols, error: perErr } = await db
  ...
  const { data: ligneCols, error: ligneErr } = await db
    .from("psp_lignes")
    .select("id, statut, priorite")
    .limit(1);
  check(
    "A3. psp_lignes.statut/priorite accessibles",
    !ligneErr && Array.isArray(ligneCols),
    ligneErr?.message ?? "",
  );
  ...
  const perTranche = await run(() =>
    db
      .from("psp_ligne_patrimoine")
      .insert({ psp_ligne_id: lid, tranche_code: trancheA, niveau: "tranche" })
      .select("id")
      .single(),
  );
  ...
  const incoherent = await run(() =>
    db
      .from("psp_ligne_patrimoine")
      .insert({ psp_ligne_id: lid, tranche_code: trancheB, niveau: "lot", lot_id: lotA1.id }),
  );
  ...
  const sansLot = await run(() =>
    db
      .from("psp_ligne_patrimoine")
      .insert({ psp_ligne_id: lid, tranche_code: trancheA, niveau: "lot" }),
  );
  ...
  const env = await run(() =>
    db
      .from("psp_enveloppes")
      .insert({ programmation_id: pid, annee: 2027, categorie: "GE", montant: 500000 })
      .select("id")
      .single(),
  );
  ...
  const envDup = await run(() =>
    db
      .from("psp_enveloppes")
      .insert({ programmation_id: pid, annee: 2027, categorie: "GE", montant: 1 }),
  );
  ...
  const envHorsPeriode = await run(() =>
    db
      .from("psp_enveloppes")
      .insert({ programmation_id: pid, annee: 2032, categorie: "GT", montant: 1000 }),
  );
  check(
    "D4. annee 2032 hors période REJETÉE (trigger)",
    !!envHorsPeriode.error,
    envHorsPeriode.msg,
  );
  ...
  const envNeg = await run(() =>
    db
      .from("psp_enveloppes")
      .insert({ programmation_id: pid, annee: 2028, categorie: "GT", montant: -5 }),
  );
  ...
  const envCat = await run(() =>
    db
      .from("psp_enveloppes")
      .insert({ programmation_id: pid, annee: 2028, categorie: "XX", montant: 5 }),
  );
  ...
  const sp = await run(() =>
    db
      .from("psp_lignes")
      .update({ statut: "attente_agence", priorite: "prioritaire" })
      .eq("id", lid)
      .select("statut, priorite")
      .single(),
  );
  check(
    "E1. statut/priorité mis à jour",
    sp.data?.statut === "attente_agence" && sp.data?.priorite === "prioritaire",
    sp.msg,
  );
  ...
  const spBadP = await run(() =>
    db.from("psp_lignes").update({ priorite: "urgent" }).eq("id", lid),
  );
  ...
  const gelPer = await run(() =>
    db
      .from("psp_ligne_patrimoine")
      .insert({ psp_ligne_id: lid, tranche_code: trancheA, niveau: "rue", rue: "RUE TEST" }),
  );
  ...
  const gelEnv = await run(() =>
    db
      .from("psp_enveloppes")
      .insert({ programmation_id: pid, annee: 2029, categorie: "CP", montant: 1000 }),
  );
  check("F3. INSERT enveloppe sur figée REJETÉ (gel)", !!gelEnv.error, gelEnv.msg);
  const gelSp = await run(() =>
    db.from("psp_lignes").update({ statut: "a_definir" }).eq("id", lid),
  );
  ...
  check(
    "G1. delete ligne → cascade périmètre",
    !delLigne && restPer.length === 0,
    `restants=${restPer.length}`,
  );
  ...
  const restProg = rows(
    await db.from("psp_programmations").select("id").ilike("remarques", `%${MARQUEUR}%`),
  );
  const restLignes = rows(
    await db.from("psp_lignes").select("id").ilike("remarques", `%${MARQUEUR}%`),
  );
