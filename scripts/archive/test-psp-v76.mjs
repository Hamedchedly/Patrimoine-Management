...
console.log("\n=== 1. BROUILLON — TR SEULE SUFFIT ===");
check(
  "TR seule (sans corps/montant/année) → enregistrable",
  brouillonEnregistrable("1977") === true,
);
...
{
  const perimetres = construirePerimetres({
    lots: [],
    adresses: [],
    rue: "RUE DE PARIS",
    mode: "auto",
  ...
  check(
    "rue seule → périmètre niveau 'rue'",
    perimetres.length === 1 &&
      perimetres[0]?.niveau === "rue" &&
      perimetres[0]?.rue === "RUE DE PARIS",
  );
  check(
    "résumé rue seule → 'Toute la rue'",
    resumeSelectionAdresse({ rue: "RUE DE PARIS", adresses: [], lots: [] }) === "Toute la rue",
  );
}
...
{
  const avant = construirePerimetres({
    lots: [],
    adresses: ["3", "5", "7"],
    rue: "RUE DE PARIS",
    mode: "auto",
  ...
  const apres = construirePerimetres({
    lots: [],
    adresses: ["3", "5", "7"],
    rue: "RUE DE PARIS",
    mode: "auto",
  ...
  check(
    "résumé conservé (3, 5, 7)",
    resumeSelectionAdresse({ rue: "RUE DE PARIS", adresses: ["3", "5", "7"], lots: [] }) ===
      "3, 5, 7",
  );
}
...
console.log("\n=== 5. SUPPRESSION DU PÉRIMÈTRE ===");
{
  ...
  check(
    "après effacement → périmètre 'tranche'",
    sansAdresse.length === 1 && sansAdresse[0]?.niveau === "tranche",
  );
  check("TR conservée (aucun état ne la touche)", brouillonEnregistrable("1977") === true);
  check(
    "résumé absent (rue effacée)",
    resumeSelectionAdresse({ rue: null, adresses: [], lots: [] }) === null,
  );
}
...
console.log("\n=== 6. ER → RUE / NUMÉRO ===");
{
  ...
  const perimetres = construirePerimetres({
    lots: [{ id: "L1", adresse, code_patrimoine: "ER.123456" }],
    adresses: [],
    rue: "RUE DE REIMS",
    mode: "auto",
  });
  check(
    "lot → périmètre niveau 'lot'",
    perimetres.length === 1 && perimetres[0]?.niveau === "lot" && perimetres[0]?.lot_id === "L1",
  );
  check(
    "résumé ER visible",
    resumeSelectionAdresse({
...
console.log("\n=== 9. TR → SOUS-SECTEUR → CC ===");
{
  ...
  check(
    "TR 1950 → CC ALOTHORE (référentiel, pas CANTONY)",
    ref.tranches.get("1950")?.charge_clientele === "ALOTHORE",
  );
...
console.log("\n=== 10. CC NON RENSEIGNÉ ===");
{
  ...
  check(
    "aucun message si CC renseigné",
    libelleCcManquant({ sous_secteur: "1", charge_clientele: "ALOTHORE" }) === null,
  );
  const tranches = [
    {
      code: "9999",
  ...
  check(
    "CC null sans référentiel (jamais fréquence)",
    ref.tranches.get("9999")?.charge_clientele === null,
  );
}
...
console.log("\n=== 11-14. RÉFÉRENTIEL CC (upsert PUR) ===");
{
  ...
  // 11. modification
  lignes = applicerReferentielCcUpsert(lignes, {
    sous_secteur: "1",
  ...
  check(
    "modification : sous-secteur 1 → CMICHEL",
    lignes.find((l) => l.sous_secteur === "1")?.charge_clientele === "CMICHEL",
  );
  ...
  check(
    "SKILIDJIAN gère 2 sous-secteurs (2 et 7)",
    lignes.filter((l) => l.charge_clientele === "SKILIDJIAN").length === 2,
  );
  ...
  check(
    "ajout CMICHEL (sous-secteur 3)",
    lignes.some((l) => l.sous_secteur === "3" && l.charge_clientele === "CMICHEL"),
  );
  check(
    "ajout LALLIANC (sous-secteur 5)",
    lignes.some((l) => l.sous_secteur === "5" && l.charge_clientele === "LALLIANC"),
  );
  // désactivation
  lignes = applicerReferentielCcUpsert(lignes, {
    sous_secteur: "2",
  ...
  check(
    "désactivation : sous-secteur 2 inactif",
    lignes.find((l) => l.sous_secteur === "2")?.actif === false,
  );
}
// ── 15-16. RÉFÉRENTIEL CORPS D'ÉTAT GE/GT/CP ──────────────────────────────────
console.log("\n=== 15-16. CORPS D'ÉTAT (RÉFÉRENTIEL) ===");
{
  ...
  check(
    "catégorie depuis référentiel (GE)",
    categorieCorpsEtatReferentiel("(f) Ravalement", referentiel) === "GE",
  );
  check(
    "catégorie depuis référentiel (CP)",
    categorieCorpsEtatReferentiel("(m) Carrelage", referentiel) === "CP",
  );
  ...
  check(
    "valeur libre hors référentiel → repli mapping (GT)",
    categorieCorpsEtatReferentiel("(k) Inconnu", referentiel) === "GT",
  );
}
...
console.log("\n=== 17-19. COMPLÉTUDE EXPORT ===");
{
  ...
  check(
    ...
    Boolean(
      mIncomplete &&
      mIncomplete.manquants.includes("Corps d'état") &&
      mIncomplete.manquants.includes("Nature travaux") &&
      mIncomplete.manquants.includes("Montant programmé (au moins une année)"),
    ),
    mIncomplete?.manquants.join(",") ?? "",
  );
  check(
    "ligne sans nature → 'Nature travaux'",
    manquants.some((l) => l.id === "op-sans-nature" && l.manquants.includes("Nature travaux")),
  );
  check(
    "ligne sans corps → 'Corps d'état'",
    manquants.some((l) => l.id === "op-sans-corps" && l.manquants.includes("Corps d'état")),
  );
  check(
    "TR portée pour chaque ligne incomplète",
    manquants.every((l) => l.tranche === "1977" || l.tranche === "1982" || l.tranche === "1991"),
  );
}
...
console.log("\n=== 20-21. ÉTAT PRÉSERVÉ ===");
{
  ...
  const p1 = construirePerimetres({
    lots: etat.lots,
    adresses: etat.adresses,
    rue: etat.rue,
    mode: "auto",
  });
  const p2 = construirePerimetres({
    lots: etat.lots,
    adresses: etat.adresses,
    rue: etat.rue,
    mode: "auto",
