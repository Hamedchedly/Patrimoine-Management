...
const { data: hist, error: e2 } = await db
  ...
  .select("numero_commande_interne, fournisseur, date_commande, montant_engage, corps_etat_libelle, patrimoine")
...
if (fix != null && !Number.isNaN(fix) && fix > 0) {
  for (const c of cmd ?? []) {
    ...
    if (Object.keys(patch).length === 0) {
      console.log(`Commande ${num} déjà renseignée (budget=${c.budget}, engage=${c.engage}) — aucun correctif.`);
      continue;
    }
