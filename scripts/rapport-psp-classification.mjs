...
const nGen = classifications.filter(
  (c) =>
    c.regle_appliquee.includes("generique") ||
    [
      "TRAVAUX GENERAUX",
      "TRAVAUX DIVERS",
      "POLYVALENCE",
...
for (const [i, g] of groupes.slice(0, 30).entries()) {
  console.log(
    `   ${String(i + 1).padStart(2)}. [${g.occurrences}x] ${g.libelle_normalise} | règle=${g.regle_appliquee} | domaine=${g.domaine_technique} | type=${g.type_intervention} | montant=${Math.round(g.montant_total)}`,
  );
}

const aValider = classifications
  .filter((c) => c.besoin_validation_humaine)
  .sort((a, b) => (b.montant_engage ?? 0) - (a.montant_engage ?? 0));
console.log(`\nK. Top 20 commandes par montant nécessitant validation :`);
for (const [i, c] of aValider.slice(0, 20).entries()) {
  console.log(
    `   ${String(i + 1).padStart(2)}. ${c.comn} | ${c.naac_source} | ${Math.round(c.montant_engage ?? 0)} € | ${c.type_intervention}/${c.domaine_technique} | ${c.libelle_normalise}`,
  );
}
