...
const { data: cmd } = await db.from("travaux_commandes").select("tranche_code, charge_clientele").eq("annee_exercice", annee).not("charge_clientele","is",null);
...
for (const c of cmd ?? []) {
  ...
  const m = freq.get(s); m.set(cc, (m.get(cc) ?? 0) + 1);
}
...
for (const [ss, m] of [...freq.entries()].sort((a,b)=>Number(a[0])-Number(b[0]))) {
  ...
  const { error } = await db.from("psp_charges_clientele").upsert(
    { sous_secteur: ss, charge_clientele: id, identifiant_personnel: id, actif: true },
    { onConflict: "sous_secteur" }
  );
  if (error) { console.error("ERR ss", ss, error.message); process.exit(1); }
