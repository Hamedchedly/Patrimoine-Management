// V8.16p — Seed des modèles de mail en base (mail_modeles) depuis MAIL_MODELES (code).
...
for (const m of MAIL_MODELES) {
  const { error } = await db.from("mail_modeles").upsert(
    { id: m.id, libelle: m.libelle, sujet: m.sujet, corps: m.corps },
    { onConflict: "id" },
  );
  if (error) {
