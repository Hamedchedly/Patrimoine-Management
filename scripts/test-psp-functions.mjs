...
// ── 1. createPspImportSchema ─────────────────────────────────────────────────
{
  ...
  assert(
    "T1 create : erreurs_detail par défaut []",
    createPspImportSchema.safeParse({
      fichier_nom: "a.xlsx",
      exercice: 2026,
      lignes_total: 1,
      lignes_valides: 1,
  ...
  assert(
    "T1 create : exercice hors bornes rejeté",
    ko(
      createPspImportSchema.safeParse({
        fichier_nom: "a.xlsx",
        exercice: 1900,
  ...
  assert(
    "T1 create : nombre négatif rejeté",
    ko(
      createPspImportSchema.safeParse({
        fichier_nom: "a.xlsx",
        exercice: 2026,
...
// ── 2. pspRowSchema ──────────────────────────────────────────────────────────
{
  ...
  assert(
    "T2  row : statut inconnu rejeté",
    ko(pspRowSchema.safeParse({ ...l, statut: "inconnu" })),
  );
  assert(
    "T2  row : niveau inconnu rejeté",
    ko(pspRowSchema.safeParse({ ...l, niveau_rattachement: "immeuble" })),
  );
  assert(
    "T2  row : numero_commande manquant rejeté",
    ko(
      pspRowSchema.safeParse(
        (() => {
          const { numero_commande, ...rest } = l;
          return rest;
        })(),
      ),
    ),
  ...
  assert(
    "T2  row : numero_commande vide accepté (ligne en erreur)",
    ok(pspRowSchema.safeParse({ ...l, numero_commande: "" })),
  );
...
// ── 3. pspBatchSchema ────────────────────────────────────────────────────────
{
  ...
  assert(
    "T3  batch : import_id non-uuid rejeté",
    ko(pspBatchSchema.safeParse({ ...batch, import_id: "abc" })),
  );
  assert("T3  batch : rows vide rejeté", ko(pspBatchSchema.safeParse({ ...batch, rows: [] })));
  assert(
    "T3  batch : annee_exercice absente acceptée",
    ok(pspBatchSchema.safeParse({ import_id: batch.import_id, rows: [ligneValide()] })),
  );
}
...
// ── 4. failPspImportSchema ───────────────────────────────────────────────────
{
  const fail = {
    import_id: "11111111-1111-4111-8111-111111111111",
    erreur_message: "Fichier illisible",
  };
  assert("T4  fail : payload valide", ok(failPspImportSchema.safeParse(fail)));
  assert(
    "T4  fail : erreur_message requis",
    ko(failPspImportSchema.safeParse({ import_id: fail.import_id })),
  );
  assert(
    "T4  fail : erreur_message vide rejeté",
    ko(failPspImportSchema.safeParse({ import_id: fail.import_id, erreur_message: "" })),
  );
}
...
// ── 5. finalizePspImportSchema ───────────────────────────────────────────────
{
  ...
  assert(
    "T5  finalize : statut termine valide",
    ok(finalizePspImportSchema.safeParse({ ...fin, statut: "termine" })),
  );
  assert(
    "T5  finalize : statut erreur valide",
    ok(finalizePspImportSchema.safeParse({ ...fin, statut: "erreur" })),
  );
  assert(
    "T5  finalize : statut inconnu rejeté",
    ko(finalizePspImportSchema.safeParse({ ...fin, statut: "annule" })),
  );
}
...
// ── 6. savePspCommandAnalysisSchema ──────────────────────────────────────────
{
  ...
  assert(
    "T6  analyse : numero_commande null accepté (COMC_NOLIG nullable)",
    ok(savePspCommandAnalysisSchema.safeParse({ ...base, numero_commande: null })),
  );
  assert(
    "T6  analyse : numero_commande absent accepté",
    ok(
      savePspCommandAnalysisSchema.safeParse(
        (() => {
          const { numero_commande, ...rest } = base;
          return rest;
        })(),
      ),
    ),
  ...
  assert(
    "T6  analyse : numero_commande_interne présent",
    ok(savePspCommandAnalysisSchema.safeParse({ ...base, numero_commande_interne: "4638553" })),
  );
  assert(
    "T6  analyse : import_row_id non-uuid rejeté",
    ko(savePspCommandAnalysisSchema.safeParse({ ...base, import_row_id: "abc" })),
  );
  assert(
    "T6  analyse : categorie_budget_statut inconnu rejeté",
    ko(savePspCommandAnalysisSchema.safeParse({ ...base, categorie_budget_statut: "inconnu" })),
  );
  assert(
    "T6  analyse : categorie_budget_statut absent accepté (dérivé côté serveur)",
    ok(
      savePspCommandAnalysisSchema.safeParse(
        (() => {
          const { categorie_budget_statut, ...rest } = base;
          return rest;
        })(),
      ),
    ),
  ...
  assert(
    "T6  analyse : confiance > 1 rejetée",
    ko(savePspCommandAnalysisSchema.safeParse({ ...base, confiance: 1.2 })),
  );
  assert(
    "T6  analyse : confiance négative rejetée",
    ko(savePspCommandAnalysisSchema.safeParse({ ...base, confiance: -0.1 })),
  );
  assert(
    "T6  analyse : source_import_id non-uuid rejeté",
    ko(savePspCommandAnalysisSchema.safeParse({ ...base, source_import_id: "abc" })),
  );
  assert(
    "T6  analyse : analyse_json par défaut {}",
    JSON.stringify(
      savePspCommandAnalysisSchema.safeParse(
        (() => {
          const { analyse_json, ...rest } = base;
          return rest;
        })(),
      ).data?.analyse_json,
    ) === "{}",
  ...
  assert(
    "T6  analyse : statut par défaut 'propose'",
    savePspCommandAnalysisSchema.safeParse(
      (() => {
        const { statut, ...rest } = base;
        return rest;
      })(),
    ).data?.statut === "propose",
  );
  assert(
    "T6  analyse : niveau par défaut 'unknown'",
    savePspCommandAnalysisSchema.safeParse(
      (() => {
        const { niveau_rattachement, ...rest } = base;
        return rest;
      })(),
    ).data?.niveau_rattachement === "unknown",
  );
  // Règle : COMC_NOLIG identique pour plusieurs COMN_NUM → autorisé (schema).
  assert(
    "T6  analyse : 2 analyses même COMC_NOLIG / COMN_NUM différents acceptées",
    ok(
      savePspCommandAnalysisSchema.safeParse({
        ...base,
        numero_commande_interne: "4638553",
        numero_commande: "0267/2023",
      }),
    ),
  );
...
// ── 7. savePspPatrimoineContextSchema ────────────────────────────────────────
{
  ...
  assert(
    "T7  contexte : niveau inconnu rejeté (pas unknown/ambiguous)",
    ko(savePspPatrimoineContextSchema.safeParse({ ...base, niveau: "unknown" })),
  );
  assert(
    "T7  contexte : er_id requis",
    ko(savePspPatrimoineContextSchema.safeParse({ ...base, er_id: "" })),
  );
  assert(
    "T7  contexte : exception par défaut false",
    savePspPatrimoineContextSchema.safeParse(
      (() => {
        const { exception, ...rest } = base;
        return rest;
      })(),
    ).data?.exception === false,
  );
  assert(
    "T7  contexte : donnees_contextuelles par défaut {}",
    JSON.stringify(
      savePspPatrimoineContextSchema.safeParse(
        (() => {
          const { donnees_contextuelles, ...rest } = base;
          return rest;
        })(),
      ).data?.donnees_contextuelles,
    ) === "{}",
  );
}
...
// ── 8. savePspFeedbackSchema ─────────────────────────────────────────────────
{
  ...
  assert(
    "T8  feedback : cible_type inconnu rejeté",
    ko(savePspFeedbackSchema.safeParse({ ...base, cible_type: "auto" })),
  );
  assert(
    "T8  feedback : cible_id requis",
    ko(savePspFeedbackSchema.safeParse({ ...base, cible_id: "" })),
  );
  assert(
    "T8  feedback : correction absente acceptée",
    ok(
      savePspFeedbackSchema.safeParse(
        (() => {
          const { correction, ...rest } = base;
          return rest;
        })(),
      ),
    ),
...
// ── 9. resoudreContexteHerite ────────────────────────────────────────────────
{
  ...
  const res = resoudreContexteHerite(
    {
      type_patrimoine: null,
      date_reference_gestion: null,
      source_date_reference: null,
      perimetre_psp: null,
    },
    parent,
  );
  assert(
    "T9  héritage : champs null complétés par le parent",
    res.type_patrimoine === "ancien" &&
      res.date_reference_gestion === "2015-03-01" &&
      res.source_date_reference === "date_debut_gestion" &&
      res.perimetre_psp === true,
  );
  ...
  assert(
    "T9  héritage : valeur propre conservée",
    res2.type_patrimoine === "neuf" && res2.perimetre_psp === false,
  );
  assert(
    "T9  héritage : null uniquement complété",
    res2.date_reference_gestion === "2015-03-01" &&
      res2.source_date_reference === "date_reception_logements",
  );
  ...
  assert(
    "T9  héritage : sans parent, inchangé",
    res3.type_patrimoine === "neuf" &&
      res3.date_reference_gestion === "2021-01-15" &&
      res3.source_date_reference === null &&
      res3.perimetre_psp === null,
  );
}
...
// ── 10. buildPspImportRowInsert ──────────────────────────────────────────────
{
  ...
  assert(
    "T10 insert : import_id propagé",
    insert.import_id === "11111111-1111-4111-8111-111111111111",
  );
  assert("T10 insert : statut propagé", insert.statut === "valide");
  assert(
    "T10 insert : donnees_brutes = ligne complète",
    JSON.stringify(insert.donnees_brutes) === JSON.stringify(ligne),
  );
  assert(
    "T10 insert : erreurs = erreurs_psp de la ligne",
    JSON.stringify(insert.erreurs) === JSON.stringify(ligne.erreurs_psp),
  );
  ...
  const sansNumero = buildPspImportRowInsert("11111111-1111-4111-8111-111111111111", null, {
    ...ligne,
    numero_commande: "",
  });
...
// ── 11. buildPspImportRowInsert — mapping ISIS complet ───────────────────────
{
  ...
  assert(
    "T11 ISIS : numero_commande_interne ← COMN_NUM",
    ins.numero_commande_interne === "4572981",
  );
  ...
  assert(
    "T11 ISIS : donnees_brutes complètes",
    JSON.stringify(ins.donnees_brutes) === JSON.stringify(ligneIsis()),
  );
}
...
// ── 12. Règles COMN_NUM / COMC_NOLIG ─────────────────────────────────────────
{
  ...
  assert(
    "T12 même COMC_NOLIG / COMN_NUM différents : batch accepté",
    ok(pspBatchSchema.safeParse({ import_id: uuid1, rows: [a, b] })),
  );
  ...
  assert(
    "T12 même COMN_NUM dans 2 imports différents : autorisé",
    ok(pspBatchSchema.safeParse(imp1)) && ok(pspBatchSchema.safeParse(imp2)),
  );
}
