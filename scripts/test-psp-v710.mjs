...
check(
  "état initial des opérations VIDE (pas PSP_OPERATIONS)",
  /useState<PspOperation[]>\[\]/.test(route.replace(/\s+/g, " ")) ||
    route.includes("useState<PspOperation[]>([])"),
);
check("PSP_OPERATIONS non importé dans la route", !route.includes("PSP_OPERATIONS"));
check(
  "état « Chargement de la programmation… » présent",
  route.includes("Chargement de la programmation"),
);
check(
  "chargement conditionné au brouillon (brouillonChargement)",
  route.includes("brouillonChargement ?"),
);
...
console.log("\n=== E/F. BUDGET ===");
{
  ...
  check(
    "budget disponible 2027 = GE+GT+CP = 440",
    budgetDisponibleParAnnee(2027, enveloppes) === 440,
  );
  check("total = somme des années (440 + 0 = 440)", budgetDisponibleTotalReel(enveloppes) === 440);
  check(
    "absence d'enveloppe → 0 (jamais de fallback 3,2 M€)",
    budgetDisponibleParAnnee(2028, {}) === 0,
  );
  check(
    "PspKpi ne passe plus PSP_BUDGET_DISPONIBLE_PAR_ANNEE",
    !kpi.includes("PSP_BUDGET_DISPONIBLE_PAR_ANNEE"),
  );
  check(
    "PspKpi affiche « — » si budget = 0",
    kpi.includes('budgetDisponible > 0 ? money0(budgetDisponible) : "—"'),
  );
}
...
console.log("\n=== H. UNE SEULE CHECKBOX DEVIS ===");
check(
  "fiche : case Devis Oui/Non unique",
  (devisPanel.match(/type="checkbox"/g) ?? []).length === 1,
);
check("fiche : bouton « Ajouter un devis » supprimé", !devisPanel.includes("Ajouter un devis"));
check(
  "fiche : le formulaire est piloté par la case (devisOui || ajoutOuvert)",
  devisPanel.includes("devisOui || ajoutOuvert"),
);
check(
  "saisie directe : une seule case Devis (basculerDevis)",
  quickAdd.includes("basculerDevis(e.target.checked)"),
);

// ── I. DEVIS SANS MONTANT ──────────────────────────────────────────────────────
console.log("\n=== I. DEVIS SANS MONTANT ===");
check(
  "fiche : montant null quand vide (ajouter)",
  devisPanel.includes('montant: montant.trim() === "" ? null : Number(montant)'),
);
check(
  "fiche : montant null quand vide (enregistrer)",
  devisPanel.includes('montant: editForm.montant.trim() === "" ? null : Number(editForm.montant)'),
);
check(
  "route : createPspDevis reçoit montant nullable",
  route.includes("montant: d.montant ?? null,"),
);
check(
  "saisie directe : montant null quand vide",
  quickAdd.includes('montant: devisMontant.trim() === "" ? null : Number(devisMontant)'),
);

// ── J/K. DATES DEMANDE / DEVIS ─────────────────────────────────────────────────
console.log("\n=== J/K. DATES ===");
check(
  "type PspDevis expose date_demande (= created_at)",
  source("src/lib/psp.prep.ts").includes("date_demande?: string | null;"),
);
check(
  "route mappe created_at → date_demande",
  route.includes('date_demande: (d["created_at"] as string | null) ?? null,'),
);
check(
  "fiche affiche « Demande le » distinct de « Devis le »",
  devisPanel.includes("Demande le") && devisPanel.includes("Devis le"),
);
check(
  "date_devis reste nullable (montant+date distincts)",
  source("src/lib/psp.prep.ts").includes("date_devis?: string | null;"),
);
...
console.log("\n=== N/O. EXPORT ===");
{
  const op = creerOperation(
    {
      ...
      ville: "",
      nature_travaux: "Carrelage",
      annee: 2027,
      programme: [12000, 0, 0, 0, 0],
    },
    "op-ex",
  );
  ...
  check(
    "Arl/sect uppercase",
    donnees.lignes[0]?.[1] === String(donnees.lignes[0]?.[1]).toUpperCase(),
  );
  check(
    "aucun « Total » / « 2026 » / « CC » dans l'export",
    !ENTETES_EXPORT_XLSX.includes("Total") &&
      !ENTETES_EXPORT_XLSX.includes("2026") &&
      !ENTETES_EXPORT_XLSX.includes("CC"),
  );
}
...
console.log("\n=== P. CC ===");
{
  ...
  check(
    "CC = référentiel (ALOTHORE, pas fréquence CANTONY)",
    ref.tranches.get("1950")?.charge_clientele === "ALOTHORE",
  );
  check(
    "ID = identifiant_personnel (CMICHEL)",
    ref.tranches.get("1950")?.identifiant_personnel === "CMICHEL",
  );
  const sansRef = construireReferencePatrimoine(tranches, [], commandes, []);
  check(
    "sans référentiel → CC null (jamais fréquence commandes)",
    sansRef.tranches.get("1950")?.charge_clientele === null,
  );
}
