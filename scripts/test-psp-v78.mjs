...
check(
  "ligne de saisie disponible sans TR (le TR est demandé à la saisie)",
  brouillonEnregistrable(null) === false,
);
...
console.log("\n=== B. AJOUT TR SEULE ===");
{
  ...
  check(
    "export la signale incomplète (contrôle à l'export, pas à la saisie)",
    manquants.length === 1,
  );
  check("corps d'état flaggé à l'export", manquants[0]?.manquants.includes("Corps d'état"));
  check(
    "montant flaggé à l'export",
    manquants[0]?.manquants.includes("Montant programmé (au moins une année)"),
  );
}
...
console.log("\n=== C. BUDGET ===");
{
  ...
  check(
    "budget disponible 2027 = somme enveloppes (440)",
    budgetDisponibleParAnnee(2027, enveloppes) === 440,
    String(budgetDisponibleParAnnee(2027, enveloppes)),
  );
  check(
    "2028 avec une seule enveloppe (GE=150) → budget disponible 150",
    budgetDisponibleParAnnee(2028, enveloppes) === 150,
  );
  check(
    "année sans enveloppe → repli dotation par défaut",
    budgetDisponibleParAnnee(2029, {}, { 2029: 3200000 }) === 3200000,
  );
  check(
    "total réel = somme des années (440 + 150 = 590)",
    budgetDisponibleTotalReel(enveloppes) === 590,
  );
  const ops = [
    creerOperation(
      {
  ...
  check(
    "cas normal : restant 80000, 84 %",
    ok.restant === 80000 &&
      ok.depassement === false &&
      Math.round((ok.pourcentage ?? 0) * 100) === 84,
  );
}
...
console.log("\n=== F. UNE SEULE COLONNE CC ===");
{
  const chemin = fileURLToPath(
    new URL("../src/components/preparation-psp/PspTable.tsx", import.meta.url),
  );
  const source = readFileSync(chemin, "utf8");
  const cc = (source.match(/label: "CC"/g) ?? []).length;
  check("une seule colonne « CC » dans le tableau", cc === 1, String(cc));
  check(
    "aucune colonne « Sous-secteur » / « Code CC »",
    !source.includes('label: "Sous-secteur"') && !source.includes('label: "Code CC"'),
  );
  check(
    "aucune colonne CC dans l'export XLSX",
    !ENTETES_EXPORT_XLSX.includes("CC") && !ENTETES_EXPORT_XLSX.includes("Chargé"),
  );
}
