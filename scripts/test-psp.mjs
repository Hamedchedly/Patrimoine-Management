...
// ── 2. COMC_NOLIG absent (COMN_NUM présent) ─────────────────────────────────
{
  const parsed = parsePspWorkbook(
    workbook(H, [
      [null, "ER.T1", null, "", "NAT", null, "Sans numéro", null, null, null, "5027001"],
    ]),
  );
  const row = parsed.lignes[0];
  assert("T2  sans COMC_NOLIG : enregistrement créé (non supprimé)", parsed.lignes.length === 1);
  assert(
    "T2  sans COMC_NOLIG : numero_commande vide (attribut nullable)",
    row?.numero_commande === "",
  );
  assert(
    "T2  sans COMC_NOLIG : numero_commande_interne conservé (COMN_NUM)",
    row?.numero_commande_interne === "5027001",
  );
  assert("T2  sans COMC_NOLIG : PAS une erreur", row?.statut !== "erreur");
  assert(
    "T2  sans COMC_NOLIG : aucune anomalie commande_manquante",
    row?.erreurs_psp.some((i) => i.code === "commande_manquante") !== true,
  );
...
// ── 4b. Même COMC_NOLIG pour plusieurs COMN_NUM → NON fusionnés ──────────────
{
  ...
  assert(
    "T4b même COMC_NOLIG / COMN_NUM différents : 2 enregistrements",
    parsed.lignes.length === 2,
  );
  assert("T4b mêmes COMC_NOLIG : 0 doublon (clé = COMN_NUM)", parsed.doublons.length === 0);
  assert(
    "T4b chaque enregistrement conserve son COMN_NUM",
    parsed.lignes.map((l) => l.numero_commande_interne).join(",") === "5027280,5027287",
  );
  assert(
    "T4b chaque enregistrement conserve le COMC_NOLIG",
    parsed.lignes.every((l) => l.numero_commande === "0559/2026"),
  );
}
...
// ── 5. Extraction ER (colonne structurée) ────────────────────────────────────
{
  const parsed = parsePspWorkbook(
    workbook(H, [
      ["GE", "ER.T123", null, "2024-004", "NAT", null, "Travaux toiture", null, null, null],
    ]),
  );
  const row = parsed.lignes[0];
...
}
{
  const parsed = parsePspWorkbook(
    workbook(H, [
      ["GE", null, "ER.B45", "2024-004b", "NAT", null, "Travaux bâtiment", null, null, null],
    ]),
  );
  const row = parsed.lignes[0];
...
// ── 6. Plusieurs ER (ambigu) ─────────────────────────────────────────────────
{
  const parsed = parsePspWorkbook(
    workbook(H, [
      [
        ...
        null,
        null,
        "Intervention ER.T88 et ER.E12 bâtiments",
        null,
        null,
        null,
      ],
  ...
  assert(
    "T6  ER multiples : er_reference = null (aucun choix arbitraire)",
    row?.er_reference === null,
  );
...
// ── 7. Code corps d'état ─────────────────────────────────────────────────────
{
  const parsed = parsePspWorkbook(
    workbook(H, [
      ["GE", "ER.T1", null, "2024-006", null, "(j) Couvertures", null, null, null, null],
    ]),
  );
  const row = parsed.lignes[0];
  ...
  assert(
    "T7b corps d'état format inversé : libellé = Couvertures",
    trailing.libelle === "Couvertures",
  );
...
// ── 8. Montant invalide (texte dans colonne numérique) ───────────────────────
{
  ...
  assert(
    "T8  montant invalide : montant_financier_valide = false",
    row?.montant_financier_valide === false,
  );
...
// ── 10. En-tête avec accents / casse / espaces ───────────────────────────────
{
  ...
  const parsed = parsePspWorkbook(
    workbook(accents, [
      ["GE", "ER.T1", null, "2024-009", "NAT", "(o) Plomberie", "Réparation", 5000, 4000, 2000],
    ]),
  );
  const row = parsed.lignes[0];
...
// ── 13. Montant négatif ──────────────────────────────────────────────────────
{
  ...
  assert(
    "T13 montant négatif : montant_financier_valide = false",
    row?.montant_financier_valide === false,
  );
  assert(
...
// ── 14. Montant incohérent (engagement > budget) ─────────────────────────────
{
  ...
  const row = parsed.lignes[0];
  assert(
    "T14 incohérence : montant_financier_valide = false",
    row?.montant_financier_valide === false,
  );
  assert(
...
{
  assert(
    "T15 normalize : 'N° COMMANDE' → 'n commande'",
    normalizePspHeader("N° COMMANDE") === "n commande",
  );
  ...
  assert(
    "T15 money : vide → null sans erreur",
    parsePspMoney("").nombre === null && parsePspMoney("").invalide === false,
  );
  assert(
    "T15 ER : 'ER.T1' détecté",
    extractErReferences([{ champ: "descriptif", valeur: "Travaux ER.T1" }]).length === 1,
  );
  assert(
    "T15 ER : 'ER.' seul ignoré",
    extractErReferences([{ champ: "descriptif", valeur: "réf ER. bâtiment" }]).length === 0,
  );
}
...
// ── 16-19. En-têtes ISIS (vrai fichier) ──────────────────────────────────────
{
  ...
  assert(
    "T16 ISIS : COMC_NOLIG.Ana_comd_trav_er → numero_commande",
    resolveHeaderAlias("COMC_NOLIG.Ana_comd_trav_er").normalizedField === "numero_commande",
  );
  assert(
    "T16 ISIS : No commande → numero_commande",
    resolveHeaderAlias("No commande").normalizedField === "numero_commande",
  );
  assert(
    "T16 ISIS : N° COMMANDE → numero_commande",
    resolveHeaderAlias("N° COMMANDE").normalizedField === "numero_commande",
  );
  assert(
    "T16 ISIS : NUMERO COMMANDE → numero_commande",
    resolveHeaderAlias("NUMERO COMMANDE").normalizedField === "numero_commande",
  );
  assert(
    "T16 ISIS : WPATRIMOINE.Ana_comd_trav_er → patrimoine",
    resolveHeaderAlias("WPATRIMOINE.Ana_comd_trav_er").normalizedField === "patrimoine",
  );
  assert(
    "T16 ISIS : COMD_DATE.Ana_comd_trav_er → date_commande",
    resolveHeaderAlias("COMD_DATE.Ana_comd_trav_er").normalizedField === "date_commande",
  );
  assert(
    "T16 ISIS : COMC_ETAT.Ana_comd_trav_er → etat",
    resolveHeaderAlias("COMC_ETAT.Ana_comd_trav_er").normalizedField === "etat",
  );
  assert(
    "T16 ISIS : W_MT_RAPPRO.Ana_comd_trav_er → engage",
    resolveHeaderAlias("W_MT_RAPPRO.Ana_comd_trav_er").normalizedField === "engage",
  );
  assert(
    "T16 ISIS : nom original conservé",
    resolveHeaderAlias("COMC_NOLIG.Ana_comd_trav_er").sourceColumn ===
      "COMC_NOLIG.Ana_comd_trav_er",
  );
  assert(
    "T16 ISIS : colonne inconnue → null",
    resolveHeaderAlias("WINDCOL.Ana_comd_trav_er").normalizedField === null,
  );
}
...
// ── 17. Parsing d'un classeur aux en-têtes ISIS ──────────────────────────────
{
  ...
  assert(
    ...
    parsed.mapping_colonnes.some(
      (m) =>
        m.normalizedField === "numero_commande" && m.sourceColumn === "COMC_NOLIG.Ana_comd_trav_er",
    ),
  );
  ...
  assert(
    "T17 ISIS : numero_commande_interne conservé (non clé)",
    row?.numero_commande_interne === "4580034",
  );
  assert("T17 ISIS : er_reference = ER.T1427 (WPATRIMOINE)", row?.er_reference === "ER.T1427");
  assert(
    "T17 ISIS : statut a_controler (corps d'état WNATURE sans code)",
    row?.statut === "a_controler",
  );
  assert(
...
{
  assert(
    "T18 ISIS : COMC_NOLIG seul → numero_commande",
    resolveHeaderAlias("COMC_NOLIG").normalizedField === "numero_commande",
  );
  assert(
    "T18 ISIS : 'comc_nolig.Ana_comd_trav_er' (casse mixte) → numero_commande",
    resolveHeaderAlias("comc_nolig.Ana_comd_trav_er").normalizedField === "numero_commande",
  );
  assert(
    "T18 ISIS : 'W PATRIMOINE.Ana_comd_trav_er' (espace) → patrimoine",
    resolveHeaderAlias("W PATRIMOINE.Ana_comd_trav_er").normalizedField === "patrimoine",
  );
  assert(
    "T18 ISIS : variante suffixe différent .AutreExport → numero_commande",
    resolveHeaderAlias("COMC_NOLIG.AutreExport").normalizedField === "numero_commande",
  );
}
...
{
  assert(
    "T19 catégorie : GE → valide",
    getCategorieBudget("GE").categorie === "GE" && getCategorieBudget("GE").statut === "valide",
  );
  assert(
    "T19 catégorie : GT → valide",
    getCategorieBudget("GT").categorie === "GT" && getCategorieBudget("GT").statut === "valide",
  );
  assert(
    "T19 catégorie : CP → valide",
    getCategorieBudget("CP").categorie === "CP" && getCategorieBudget("CP").statut === "valide",
  );
  assert(
    "T19 catégorie : AC → a_confirmer (non converti)",
    getCategorieBudget("AC").categorie === "AC" &&
      getCategorieBudget("AC").statut === "a_confirmer",
  );
  assert(
    "T19 catégorie : HO → a_confirmer (non converti)",
    getCategorieBudget("HO").categorie === "HO" &&
      getCategorieBudget("HO").statut === "a_confirmer",
  );
  assert(
    "T19 catégorie : autre valeur → a_confirmer",
    getCategorieBudget("XYZ").categorie === "XYZ" &&
      getCategorieBudget("XYZ").statut === "a_confirmer",
  );
  assert(
    "T19 catégorie : null → categorie null + a_confirmer",
    getCategorieBudget(null).categorie === null &&
      getCategorieBudget(null).statut === "a_confirmer",
  );
  assert(
    "T19 catégorie : vide → categorie null + a_confirmer",
    getCategorieBudget("").categorie === null && getCategorieBudget("").statut === "a_confirmer",
  );
  assert(
    "T19 catégorie : casse insensible (ge) → GE valide",
    getCategorieBudget("ge").categorie === "GE" && getCategorieBudget("ge").statut === "valide",
  );
}
...
// ── 20. Chargé d'opération (UTIC_CODE.Ana_comd_trav_er → charge_operation) ──
{
  ...
  assert(
    "T20 UTIC : charge_operation extrait (CHARGE-A)",
    l1?.charge_operation === "CHARGE-A",
    String(l1?.charge_operation),
  );
  ...
  assert(
    ...
    l1 &&
      Object.prototype.hasOwnProperty.call(l1, "UTIC_CODE.Ana_comd_trav_er") &&
      l1["UTIC_CODE.Ana_comd_trav_er"] === "CHARGE-A",
  );
  assert(
    "T20 UTIC : mapping_colonnes débogable",
    parsed.mapping_colonnes.some(
      (m) =>
        m.sourceColumn === "UTIC_CODE.Ana_comd_trav_er" && m.normalizedField === "charge_operation",
    ),
  );
  ...
  const demo = parsePspWorkbook(
    workbook(H, [
      ...
      [
        ...
        null,
        "Peinture",
        "Travaux ER.T103 et ER.E4 — montant en attente",
        "12 000",
        -500,
        300,
      ],
